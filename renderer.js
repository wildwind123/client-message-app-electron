...
let runListeners = () => {
    ...
    function haederListener(event) {
        ...
        let style = target.parentElement.querySelector('ul')
            .style
    ...
    const inputEl = document.querySelector('.app-message-input')
        .querySelector('input')
    inputEl.addEventListener('keypress', function(event) {
        if (event.key === 'Enter') {
            ...
            } = faker.getServerRoomChannelId(lastSelectedServerId)
            faker.addNewMessage(
    ...
    const currentUserInfoAvatarEl = document.querySelector('.app-column-chatlist-footer')
        ?.querySelector('.app-user:not(.hide)')
        ?.querySelector('.app-avatar')
    ...
    const usersInfoEl = document.querySelector('#app-list-root-right')
        ?.querySelectorAll('.app-user-info-three:not(.hide)')
    if (usersInfoEl !== undefined && usersInfoEl !== null) {
        usersInfoEl.forEach((el) => {
            el.querySelectorAll('.app-user')
                ?.forEach(
                    (element) => {
...
const renderServer = (id, force) => {
    ...
    const {
        lastSelectedRoomId,
        lastSelectedChannelId
    } = faker.getServerRoomChannelId(id)
...
let renderRoom = (roomId) => {
    ...
    newElementRoom.querySelector('.app-list-header')
        .innerHTML = room.name
    newElementRoom.id = "room-id-" + room.id
    if (faker.roomIsHide(lastSelectedServerId, room.id) === true) {
        newElementRoom.querySelector('.app-list-header')
            .classList.remove('app-list-header-hide')
        newElementRoom.querySelector('.app-list-header')
            .classList.add('app-list-header-show')
        newElementRoom.querySelector('ul')
            .style.display = 'none'
    }
...
let clearUserList = () => {
    let nodes = document.getElementById('app-list-root-right')
        .querySelectorAll('.app-list:not(.hide)')
    if (nodes === null) {
...
let renderMessage = (id) => {
    ...
    newMessageEl.querySelector('.app-column-chat-message-username')
        .innerHTML = user?.name
    newMessageEl.querySelector('.app-column-chat-message-text')
...
let renderEffectChannelSelected = (roomId, channelId) => {
    let selectedList = document.querySelector('.app-room')
        .querySelectorAll('.app-list-selected');
    ...
    if (roomIdEl === null) {
        ...
        roomIdEl.querySelector('.app-list-header')
            .classList.add('app-list-selected')
...
let renderCurrentUserInfo = (userId) => {
    ...
    currentUserEl.querySelector('.app-username')
        .innerHTML = user.name
    currentUserEl.querySelector('.app-user-info-text')
        .innerHTML = 'ID: ' + user.id
    currentUserEl.querySelector('.app-avatar')
        .style.backgroundImage = "url('style/css/" + user.icon + "')"
    if (user.online === false) {
        currentUserEl.querySelector('.app-avatar')
            .classList.remove('app-avatar-online')
        currentUserEl.querySelector('.app-avatar')
            .classList.add('app-avatar-offline')
    } else {
        currentUserEl.querySelector('.app-avatar')
            .classList.remove('app-avatar-offline')
        currentUserEl.querySelector('.app-avatar')
            .classList.add('app-avatar-online')
    }
...
let renderUsersInfo = (userIds) => {
    ...
    let renderUsers = (header, users) => {
        ...
        let userInfoTemplate = nemOnlineEl.querySelector('.app-user-info-root')
            .cloneNode(true)
        
        nemOnlineEl.querySelector('.app-user-info-root')
            .remove()
        nemOnlineEl.classList.remove('hide')
        
        nemOnlineEl.querySelector('.app-user-info-header')
            .innerHTML = header
        ...
        for (let userKey in users) {
            ...
            if (user.id === selectedUserId) {
                newUserInfo.querySelector('.app-user')
                    .classList.add('app-current-user')
            }
            newUserInfo.querySelector('.app-user-info-text-name')
                .innerHTML = user.name
            ...
            avatar.classList.remove('app-avatar-offline')
            if (user.online === true) {
                newUserInfo.querySelector('.app-user-info-text')
                    .innerHTML = user.game
                avatar.classList.add('app-avatar-online')
            } else {
                newUserInfo.querySelector('.app-user-info-text')
...
let clearServerImages = () => {
    let images = document.querySelector('.app-column-servers')
        .querySelectorAll('.app-avatar:not(.hide)')
    if (images === null) {
...
let clearUsersInfo = () => {
    let userLists = document.querySelector('#app-list-root-right')
        .querySelectorAll('.app-user-info-three:not(.hide)')
...
let clearServer = () => {
    document.querySelector('.app-column-channel-name')
        .innerHTML = '';
    document.querySelector('.app-column-chatlist-footer')
        ?.querySelector('.app-user:not(.hide)')
        ?.remove()
    clearChannels()
    clearMessages()
